...
function rbw_list() {
    ...
    local fzf_args=(
        ...
        --bind="ctrl-u:execute-silent(rbw get {} --field username | wl-copy -o)+execute-silent(touch ${TIMESTAMP_FILE})+transform-input-label(echo \"Copied username\")"
        --bind="ctrl-p:execute-silent(rbw get {} --field password | wl-copy -o)+execute-silent(touch ${TIMESTAMP_FILE})+transform-input-label(echo \"Copied password\")"
        --bind="ctrl-o:execute-silent(rbw code {} | wl-copy -o)+execute-silent(touch ${TIMESTAMP_FILE})+transform-input-label(echo \"Copied OTP\")"
        --bind="ctrl-r:execute-silent(rbw get {} --raw | jq -r --arg category \"Recovery\" '.fields[]? | select (.name == \$category) | .value' | wl-copy -o)+execute-silent(touch ${TIMESTAMP_FILE})+transform-input-label(echo \"Copied recovery code\")"
